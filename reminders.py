    ...
    async def check_personal_deadlines(self):
        ...
        try:
            ...
            for deadline in deadlines:
                ...
                elif TimeManager.is_in_reminder_window(deadline.deadline, "hour"):
                    if user.notify_hour and not deadline.reminded_hour:
                        await self.send_personal_reminder(user.telegram_id, deadline, "час")
                        deadline.reminded_hour = True
                        session.commit()
    ...
    async def check_group_deadlines(self):
        ...
        try:
            ...
            for deadline in deadlines:
                ...
                for reminder_type in ["week", "day", "hour"]:
...
async def setup_reminder_job(application):
    ...
    # Запускаем задачу каждые 1 минуту
